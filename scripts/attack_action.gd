...
func execute(level: Level) -> bool:
        ...
        # Используем актуальные позиции из _occupied
        var my_pos: Vector2i = level.get_entity_position(entity)
        var target_pos: Vector2i = level.get_entity_position(target)
        
        # Проверяем дистанцию
        var distance: int = abs(my_pos.x - target_pos.x) + abs(my_pos.y - target_pos.y)
        if distance != 1:
