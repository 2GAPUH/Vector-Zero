...
func get_next_action() -> Action:
        ...
        var my_pos: Vector2i = level.get_entity_position(entity)
        ...
        # Также получаем актуальную позицию героя
        var hero_pos: Vector2i = level.get_entity_position(target_hero)
        _last_known_hero_pos = hero_pos
        
        # 2. Проверяем: герой рядом? -> Атака
        if _is_adjacent(my_pos, hero_pos):
                var attack_action: Action = _create_attack_action(level, hero_pos)
                if attack_action != null:
                        return attack_action
        
        # 3. Проверяем: есть ли препятствие на пути? -> Атака препятствия
        var obstacle_action: Action = _check_and_attack_obstacle(level, my_pos, hero_pos)
        ...
        var distance: int = _manhattan_distance(my_pos, hero_pos)
        ...
        return _create_move_action(level, hero_pos)
